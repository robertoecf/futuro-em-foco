...
export class MonteCarloSimulationService implements IMonteCarloSimulationService {
  ...
  async runPortfolioSimulation(
    ...
    const rawResult: BrownianMonteCarloResult =
      await runUltraOptimizedMonteCarloSimulationWithParams({
    ...
    const statistics = {
      ...
      standardDeviation: rawResult.statistics.standardDeviation.map((v) =>
        safeMoneyFromNumber(v, currency)
      ),
      ...
      finalAmountRange: {
        ...
        median: safeMoneyFromNumber(
          rawResult.statistics.percentile50[rawResult.statistics.percentile50.length - 1],
          currency
        ),
      },
    };
  ...
  async runRetirementSimulation(
    ...
    const sustainabilityProbability = medianFinalWealth.greaterThan(totalRetirementNeeds)
      ? 0.75
      : 0.25;
    ...
    const incomeRange = {
      pessimistic: accumulationPhase.statistics.percentile25[
        accumulationPhase.statistics.percentile25.length - 1
      ]
        .multiply(retirementReturn)
        .divide(12),
      median: medianFinalWealth.multiply(retirementReturn).divide(12),
      optimistic: accumulationPhase.statistics.percentile75[
        accumulationPhase.statistics.percentile75.length - 1
      ]
        .multiply(retirementReturn)
        .divide(12),
    };
    ...
    const successProbability =
      sustainabilityProbability * accumulationPhase.statistics.successProbability;
  ...
  async runComparisonSimulation(
    params: ComparisonSimulationParams
