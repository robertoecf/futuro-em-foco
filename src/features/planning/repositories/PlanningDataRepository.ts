...
export class LocalStoragePlanningDataRepository implements IPlanningDataRepository {
  ...
  migrateSessionData(data: unknown): PlanningData {
    ...
    const migrated: PlanningData = {
      ...
      financialInfo: {
        initialAmount:
          this.ensureMoneyObject(data as PlanningData).financialInfo?.initialAmount || Money.zero(),
        monthlyContribution:
          this.ensureMoneyObject(data as PlanningData).financialInfo?.monthlyContribution ||
          Money.zero(),
        targetRetirementIncome: (data as PlanningData).financialInfo?.targetRetirementIncome
          ? this.ensureMoneyObject(data as PlanningData).financialInfo?.targetRetirementIncome
          : undefined,
        currentSavings: (data as PlanningData).financialInfo?.currentSavings
          ? this.ensureMoneyObject(data as PlanningData).financialInfo?.currentSavings
          : undefined,
      },
      preferences: {
    ...
    if ((data as PlanningData).calculationResults) {
      migrated.calculationResults = {
        ...
        projectedWealth:
          this.ensureMoneyObject(data as PlanningData).calculationResults?.projectedWealth ||
          Money.zero(),
        sustainableIncome:
          this.ensureMoneyObject(data as PlanningData).calculationResults?.sustainableIncome ||
          Money.zero(),
  ...
  private ensureMoneyObject(value: unknown): Money {
    ...
    if (typeof value === 'object' && value !== null && 'amount' in value && 'currency' in value) {
      return Money.fromNumber(
        (value as { amount: number; currency: string }).amount,
        (value as { currency: string }).currency || 'BRL'
      );
    }
  ...
  private deserializePlanningData(data: unknown): PlanningData {
    ...
    if (result.financialInfo) {
      result.financialInfo.initialAmount = this.ensureMoneyObject(
        result.financialInfo.initialAmount
      );
      result.financialInfo.monthlyContribution = this.ensureMoneyObject(
        result.financialInfo.monthlyContribution
      );

      if (result.financialInfo.targetRetirementIncome) {
        result.financialInfo.targetRetirementIncome = this.ensureMoneyObject(
          result.financialInfo.targetRetirementIncome
        );
      }

      if (result.financialInfo.currentSavings) {
        result.financialInfo.currentSavings = this.ensureMoneyObject(
          result.financialInfo.currentSavings
        );
      }
    }
    ...
    if (result.calculationResults) {
      result.calculationResults.lastCalculatedAt = new Date(
        result.calculationResults.lastCalculatedAt
      );
      result.calculationResults.projectedWealth = this.ensureMoneyObject(
        result.calculationResults.projectedWealth
      );
      result.calculationResults.sustainableIncome = this.ensureMoneyObject(
        result.calculationResults.sustainableIncome
      );
    }
